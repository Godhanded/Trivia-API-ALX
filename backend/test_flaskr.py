...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_create_question(self):
        res = self.client().post("/questions",
                                 json={"question": "is python oop?", "answer": "yes", "difficulty": 1, "category": 1})
        data = json.loads(res.data)
        self.assertEqual(data["status"], 200)
        self.assertTrue(data["success"])
    ...
    def test_422_if_cant_create_question(self):
        res = self.client().post(
            "/questions", json={"question": "is python oop?", "answer": "yes"})
        data = json.loads(res.data)
    ...
    def test_delete_question_with_id(self):
        ...
        data = json.loads(res.data)
        self.assertEqual(data["status"], 200)
        self.assertEqual(data["question_id"], 3)
        self.assertTrue(data["success"])
    ...
    def test_run_quiz(self):
        res = self.client().post("/quizzes",
                                 json={"previous_questions": [1, 2, 3], "quiz_category": {"id": 3, "type": "title"}})
        data = json.loads(res.data)
        self.assertEqual(data["status"], 200)
        self.assertTrue(data["success"])
    ...
    def test_400_if_run_quize_fails(self):
        res = self.client().post("/quizzes", json={"previous_questions": [
            1000, 2000, 3000], "quiz_category": {"id": 3565, "type": "tithvjh jbhvh56576854le"}})
        data = json.loads(res.data)
